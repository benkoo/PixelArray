...
dim = Dim(-1.2, 1.2, 50)
M1 = make_mat((x, y) -> y - x^2, dim, dim, 0.05)
M2 = make_mat((y,z) -> y + z^2 - 1, dim, dim, .05)  # Corrected the missing argument for M2

using UnicodePlots
spy(M1)
spy(M2)
spy(M1 * M2)  # Matrix multiplication using .* instead of *